...
def find_company_contacts(
    ...
    """
    Busca TODOS os contatos de uma empresa no Lusha (endpoint /v2/company).
    Muito mais caro que find_contacts (1 crédito por contato), mas garante
    base completa verificada.

    Devolve lista de contatos ou None em caso de erro/indisponibilidade.
    """
    if not is_configured(api_key) or not domain:
        return None
    ...
    try:
        ...
        logger.info("Lusha indisponível (company search): %s", e)
        return None

    if resp.status_code == 401:
        logger.warning("Lusha recusou a chave (401).")
        return None
    if resp.status_code == 402:
        logger.warning("Lusha sem créditos (402).")
        return None
    if resp.status_code == 429:
        logger.warning("Lusha atingiu limite de requisições (429).")
        return None
    if resp.status_code == 404:
        return []  # Empresa não está na base deles
    if resp.status_code != 200:
        logger.info("Lusha company search respondeu %s", resp.status_code)
        return None
    ...
    # Payload pode ser: {"data": [...]} ou {"contacts": [...]} ou [...] direto
    contacts = payload.get("data") or payload.get("contacts") or payload.get("people") or []
    ...
    for contact in contacts:
        ...
        if emails or phones or title:
            result.append({
                ...
                "name": name,
                "title": title,
                "emails": [{"email": e, "status": "unknown", "confidence": CONF_LUSHA} for e in emails],
                "phones": phones,
                "linkedin_url": contact.get("linkedinUrl") or contact.get("linkedin_url"),
            })
